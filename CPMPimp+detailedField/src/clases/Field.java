...
public class Field 
{
	...
	private  int vacates=0;
	private  ArrayList vacatesSize=new ArrayList();
	ArrayList frames=new ArrayList();


	
	public void showVacatesSize()
	{
		int i;
		int aux;
		for (i=0;i<vacatesSize.size();i++)
		{
			aux=(int)vacatesSize.get(i);
			System.out.println(" "+aux+" ");
		}
	}
	public  int getVacates() {
		return vacates;
	}
	public  void setVacates(int vacates) {
		       ...
		       // System.out.println("Entrada: \nColumnas:"+ columnas+" Contenedores: "+contenedores);
		        ...
		        while (entrada.hasNextInt()) 
		        {
		        	...
		        	for (i=0;i<q;i++)
		        	{
		        		num=entrada.nextInt();
		        		nuevo.add(num);
		        		Contenedor contenedor=new Contenedor(num);
		        		columna.add(contenedor);
	...
	public  void atomic_Move(int c_o,int c_d) 
	{
		...
		//AQUI MUEVO LOS ELEMENTOS DE DF (DETAILED FIELD)
		ArrayList tmp;
		tmp=(ArrayList)DF.get(c_o);
		Contenedor contenedor=(Contenedor)tmp.remove(tmp.size()-1);
		tmp=(ArrayList)DF.get(c_d);
		tmp.add(contenedor);
		setCoordenadas();
	}	
	...
	public  int columnBestFill(int c_o)
	{
		...
		//EN CASO CONTRARIO
		aux=(int)tops.get(bestCol);
		ArrayList cols=new ArrayList();
		for (i=0;i<field.size();i++)
		{
			temp=(Column)field.get(i);
			//SI LA COLUMNA ESTA DESORDENADA Y CONTIENE EL MEJOR NUMERO
			if (!temp.isLock() && (int)tops.get(i)==aux)
			{
				cols.add(i);
			}
		}
		...
		cols=bubbleSort(cont, cols);
		
		//retorna el mejor movimiento
		return (int)cols.get(0);
	}
	


	public  int contador(int col)
	{
		Column temp;
		temp=(Column)field.get(col);
		int base=1;
		int top=(int)temp.get(temp.size()-base);
		int contador=1;

		while (top>=(int)temp.get(temp.size()-base) && base<=temp.size()-1)
		{
			contador++;
			//se actualiza el top
			top=(int)temp.get(temp.size()-base);
			//se actualiza base
			base++;
		}
	 ...
	 */
	/*public  void AIOfill()    VERSION ANTIGUA
	{
		int i;
		Column temp;
		ArrayList size=new ArrayList();
		ArrayList columna=new ArrayList();
		//SE LEE EL TAMAnO Y LA COLUMNA 
		for (i=0;i<field.size();i++)
		{
			temp=(Column)field.get(i);
			size.add(temp.size());
		...
		//LA IDEA ES TENER UNA LISTA CON LAS COLUMNAS ORGANIZADAS POR TAMAnO, DE MAYOR A MENOR
		...
		int aux;
		 ...
		 //UNA VEZ OBTENIDAS LAS COLUMNAS POR ORDEN DE TAMAnO (MENOR A MAYOR) SE RELLENAN
		 int c_d;
	...
	public  ArrayList lil2Big()
	{
		...
		cols=bubbleSort(size, cols);
		
		return cols;

	}
	public  void AIOEmptyFill(int c_o)
	{
		ArrayList tops=getTops();
		Column temp;
		int i;
	 ...
	 * @param c_o
	 */
	public  void AIOsacrifice(int c_o)
	{
		ArrayList size=new ArrayList();
		ArrayList columna=new ArrayList();
		int i;
	...
	public  int[] generateTiers()
	{
		...
		//SE ASIGNA TAMAnO CERO A TODOS LOS TIERS
	...
	public  void vacateColumn()
	{
		...
		//SE GUARDAN LA/LAS COLUMNAS QUE TENGAN EL TAMAnO MINIMO
		for (i=0;i<field.size();i++)
		{
			temp=(Column)field.get(i);
			if (!temp.isLock() && temp.size()==menor)
	...
	public  void AIOVacateColumn()
	{
		...
		while (temp.size()!=0)
		{
			...
			if (selector!=-1)
			{
				...
				if(temp.isLock() && !temp.isEmpty())
				{
					if (Contador(col)+temp.size()>=hMax || Contador(col)==allDisorderlyElements())
					{
						setVacates(getVacates() + 1);
						break;
	...
	public  int selectMinumCol()
	{
		...
		temp=(Column)field.get((int)columna.get(0));
		int s=temp.size();
		for (i=0;i<field.size();i++)
		{
			temp=(Column)field.get(i);
		...
		for (i=0;i<cols.size();i++)
		{
			promedio.add(prom((int)cols.get(i)));
		}
		
		cols=bubbleSort(promedio, cols);
		
		return (int)cols.get(0);
		
	}
	
	public  int prom(int c)
	{
		Column temp=(Column)field.get(c);
		int i;
		int aux;
		int sum=0;
	...
	public  int allDisorderlyElements()
	{
		ArrayList elements=new ArrayList();
		...
		for (i=0;i<field.size();i++)
		{
			...
			if (!temp.isLock())
			{
				//AQUI SE RESCATA UNICAMENTE A LOS QUE SON ESTAN DESORDENADOS
				for (j=0;j<temp.size()-1;j++)
				{
					//I
					aux=(int)temp.get(j);
					//I+1
					aux2=(int)temp.get(j+1);
					
					//EN CASO DE QUE I<I+1
					if (aux<aux2)
					{
						//LA BASE SIEMPRE ESTA TEORICAMENTE ORDENADA, POR LO TANTO ES SIZE-(J+1)  (EN CASO DE QUE J=0, ES UN ELEMENTO ORDENADO Y EL RESTO NO.
						cont=temp.size()-(j+1);
						
						//MIENTRAS CONTADOR SIGA VIVO (NO SEA 0), AnADIR UN CONTADOR SI ESTO ES SABER LA CANTIDAD DE ELEMENTOS MALOS, NO TODOS
						while (cont!=0)
						{
							elements.add(1);
							cont--;
	...
	public void setCoordenadas()
	{
		...
		for (i=0;i<DF.size();i++)
		{
			temp=(ArrayList) DF.get(i);
			for (j=0;j<temp.size();j++)
			{
				c=(Contenedor)temp.get(j);
